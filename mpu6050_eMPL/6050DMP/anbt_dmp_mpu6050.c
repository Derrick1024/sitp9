...
u8 AnBT_DMP_MPU6050_Init(void)	
{
	...
	printf("1.DMP设备初始化成功\n"); 		//完成初始化
	//
	if(!mpu_set_sensors(INV_XYZ_GYRO | INV_XYZ_ACCEL)) printf("2.DMP设置传感器成功 \n");		//完成传感去设定
	else printf("2.DMP设置传感器错误\n");
	//
	if(!mpu_configure_fifo(INV_XYZ_GYRO | INV_XYZ_ACCEL)) printf("3.设置FIFO成功 \n");		//完成fifo设置
	else printf("3.设置FIFO失败 \n");
	//
	if(!mpu_set_sample_rate(DEFAULT_MPU_HZ))printf("4.DMP设置采样率成功 \n");			//完成DMP采样率设置
	else printf("4.DMP设置采样率失败 \n");
	//
	if(!dmp_load_motion_driver_firmware())	printf("5.DMP加载运动驱动固件成功 \n");				//开启DMP功能
	else printf("5.DMP加载运动驱动固件失败 \n");
	//
	if(!dmp_set_orientation(inv_orientation_matrix_to_scalar(gyro_orientation))) printf("6.DMP完成设置初始方向设定偏差 \n");		//初始方向设定偏差
	else printf("6.DMP设置初始方向设定偏差出错 \n");
	//
	if(!dmp_enable_feature(DMP_FEATURE_6X_LP_QUAT | DMP_FEATURE_TAP |
	DMP_FEATURE_ANDROID_ORIENT | DMP_FEATURE_SEND_RAW_ACCEL | DMP_FEATURE_SEND_CAL_GYRO |
	DMP_FEATURE_GYRO_CAL))		printf("7.DMP使能成功 \n");		//DMP使能
	else printf("7.DMP使能出错 \n");
	//
	if(!dmp_set_fifo_rate(DEFAULT_MPU_HZ)) printf("8.DMP设置FIFO速率成功 \n");		//DMP  fifo速度
	else printf("8.DMP设置FIFO速率失败 \n");
	//
	run_self_test();
	if(!mpu_set_dmp_state(1)) printf("9.Dmp启动成功 \n");				//DMP成功开启
	else printf("9.Dmp启动失败 \n");					//Dmp启动失败
	//
	return 0;
}
